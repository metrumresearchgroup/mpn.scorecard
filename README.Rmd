...
## Comments (optional)
Packages with low scores that fall short of initial expectations may include a comment text file. The presence of a comment section indicates that we are aware the score is low, but are proceeding with adding the package to MPN or otherwise approving it. The rationale for doing so is included in the comment file itself. Note that this is not intended to be used to denote a deviation from expected results. Inclusion of this section works the same as a traceability matrix. If a comment file matching the expected naming convention (`<package_tarball_name>.comments.txt` or `package_3.1.0.comments.txt` in the above example) is found in `results_dir`, the section will be automatically be included.
